...
class TestDependencyValidation:
    ...
    def test_missing_ar_command_exits(self):
        ...
        with patch("sys.argv", test_args):
            ...
            with patch(
                "sysroot_maker.is_command_available", side_effect=mock_command_available
            ):
                with pytest.raises(SystemExit) as exc_info:
                    sysroot_maker.main()
                assert exc_info.value.code == 1
                ...
                with patch("sysroot_maker.is_root", return_value=True):
                    with patch("os.path.exists", return_value=True):
                        with patch("shutil.rmtree") as mock_rmtree:
                            with patch(
                                "sysroot_maker.run_debootstrap", return_value=True
                            ):
                                try:
                                    sysroot_maker.main()
                                except SystemExit:
...
@pytest.mark.unit
class TestProgressFlagParsing:
    """Test --progress flag parsing."""

    def test_cli_progress_flag_parsing(self):
        """Test that --progress flag is parsed correctly."""
        test_args = ["sysroot_maker.py", "--output", "/tmp/test", "--progress"]
        with patch("sys.argv", test_args):
            with patch("sysroot_maker.is_command_available", return_value=True):
                with patch("sysroot_maker.is_root", return_value=True):
                    with patch("sysroot_maker.run_debootstrap", return_value=True):
    ...
    def test_progress_flag_default_is_false(self):
        """Test that --progress defaults to False."""
        test_args = ["sysroot_maker.py", "--output", "/tmp/test"]
        with patch("sys.argv", test_args):
            with patch("sysroot_maker.is_command_available", return_value=True):
                with patch("sysroot_maker.is_root", return_value=True):
