...
from sysroot_maker import ProgressIndicator
...
class TestMirrorSelection:
    ...
    def test_arm64_uses_ports_mirror(self):
        assert (
            sysroot_maker.get_mirror_for_arch("arm64")
            == "http://ports.ubuntu.com/ubuntu-ports"
        )

    def test_armhf_uses_ports_mirror(self):
        assert (
            sysroot_maker.get_mirror_for_arch("armhf")
            == "http://ports.ubuntu.com/ubuntu-ports"
        )

    def test_ppc64el_uses_ports_mirror(self):
        assert (
            sysroot_maker.get_mirror_for_arch("ppc64el")
            == "http://ports.ubuntu.com/ubuntu-ports"
        )

    def test_s390x_uses_ports_mirror(self):
        assert (
            sysroot_maker.get_mirror_for_arch("s390x")
            == "http://ports.ubuntu.com/ubuntu-ports"
        )

    def test_riscv64_uses_ports_mirror(self):
        assert (
            sysroot_maker.get_mirror_for_arch("riscv64")
            == "http://ports.ubuntu.com/ubuntu-ports"
        )

    def test_amd64_uses_main_archive(self):
        assert (
            sysroot_maker.get_mirror_for_arch("amd64")
            == "http://archive.ubuntu.com/ubuntu"
        )

    def test_i386_uses_main_archive(self):
        assert (
            sysroot_maker.get_mirror_for_arch("i386")
            == "http://archive.ubuntu.com/ubuntu"
        )

    def test_unknown_arch_uses_main_archive(self):
        assert (
            sysroot_maker.get_mirror_for_arch("unknown")
            == "http://archive.ubuntu.com/ubuntu"
        )
...
class TestCommandAvailability:
    ...
    def test_command_availability_for_ar(self, monkeypatch):
        monkeypatch.setattr(
            "shutil.which", lambda cmd: "/usr/bin/ar" if cmd == "ar" else None
        )
...
class TestDiskSpaceCheck:
    ...
    def test_check_disk_space_os_error(self, monkeypatch):
        monkeypatch.setattr(
            "os.statvfs", Mock(side_effect=OSError("Permission denied"))
        )
...
class TestExtractDebPackage:
    ...
    @patch("pathlib.Path.glob")
    @patch("tempfile.TemporaryDirectory")
    def test_extract_deb_ar_extraction_fails(
        self, mock_tmpdir, mock_glob, mock_run, tmp_path
    ):
    ...
    @patch("pathlib.Path.glob")
    @patch("tempfile.TemporaryDirectory")
    def test_extract_deb_no_data_archive(
        self, mock_tmpdir, mock_glob, mock_run, tmp_path
    ):
    ...
    @patch("pathlib.Path.glob")
    @patch("tempfile.TemporaryDirectory")
    def test_extract_deb_tar_extraction_fails(
        self, mock_tmpdir, mock_glob, mock_run, tmp_path
    ):
...
class TestValidateDebArchitectureExtended:
    ...
    def test_validate_no_control_archive(self, mock_glob, mock_tmpdir, mock_run):
        ...
        result = sysroot_maker.validate_deb_architecture(
            "/path/to/package.deb", "arm64", verbose=True
        )
    ...
    def test_validate_no_architecture_field(self, mock_glob, mock_tmpdir, mock_run):
        ...
        mock_run.side_effect = [
            Mock(returncode=0),
            Mock(
                returncode=0, stdout="Package: test\nVersion: 1.0\n"
...
class TestProgressIndicator:
    ...
    def test_progress_disabled_by_default(self):
        """Test that progress is disabled by default."""
        progress = ProgressIndicator()
        assert progress.enabled is False

    def test_progress_disabled_when_not_tty(self, monkeypatch):
        """Test that progress is disabled when stdout is not a TTY."""
        monkeypatch.setattr("sys.stdout.isatty", lambda: False)
        progress = ProgressIndicator(enabled=True)
        assert progress.enabled is False

    def test_progress_enabled_when_tty_and_flag(self, monkeypatch):
        """Test that progress is enabled when TTY and flag is set."""
        monkeypatch.setattr("sys.stdout.isatty", lambda: True)
        progress = ProgressIndicator(enabled=True)
        assert progress.enabled is True

    def test_set_stage_prints_when_enabled(self, monkeypatch, capsys):
        """Test that set_stage prints stage info when enabled."""
        monkeypatch.setattr("sys.stdout.isatty", lambda: True)
        progress = ProgressIndicator(enabled=True)
        progress.set_stage(3, "Testing stage")

        captured = capsys.readouterr()
        assert "[3/6] Testing stage" in captured.out

    def test_set_stage_silent_when_disabled(self, capsys):
        """Test that set_stage is silent when disabled."""
    ...
    def test_spinner_disabled_in_verbose_mode(self, monkeypatch):
        """Test that spinner is disabled in verbose mode."""
        monkeypatch.setattr("sys.stdout.isatty", lambda: True)
        progress = ProgressIndicator(enabled=True, verbose=True)

        # Spinner context should be a no-op in verbose mode
        with progress.spinner("Test message"):
            # Should not raise any errors
            pass

    def test_spinner_works_when_enabled(self, monkeypatch):
        """Test that spinner context manager works when enabled."""
        monkeypatch.setattr("sys.stdout.isatty", lambda: True)

        # Create a mock stdout to capture spinner output
        mock_stdout = io.StringIO()
        monkeypatch.setattr("sys.stdout", mock_stdout)

        # Re-check isatty after monkeypatch
        mock_stdout.isatty = lambda: True

        progress = ProgressIndicator(enabled=True, verbose=False)

        with progress.spinner("Test message"):
            time.sleep(0.15)  # Let spinner run briefly

        # Should have written something (spinner chars + message)
        output = mock_stdout.getvalue()
        assert len(output) > 0

    def test_spinner_no_op_when_disabled(self, monkeypatch):
        """Test that spinner is a no-op when progress is disabled."""
        progress = ProgressIndicator(enabled=False)
